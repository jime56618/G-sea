...
const AGENTES_CACHE_KEY = 'agentes_list';
const ASEG_CACHE_KEY = 'aseguradoras_list';

export default function SeccionAgentes() {
  const [isSidebarExpanded, setIsSidebarExpanded] = useState(false);
  const cachedAgentes = useMemo(() => readModuleCache(AGENTES_CACHE_KEY), []);
  const [data, setData] = useState(() => (Array.isArray(cachedAgentes) ? cachedAgentes : []));
  const [aseguradoras, setAseguradoras] = useState(
    () => readModuleCache(ASEG_CACHE_KEY) || []
  );
  const [loading, setLoading] = useState(!cachedAgentes);
  ...
  const fetchAgentes = useCallback(async () => {
    try {
      ...
      if (!cachedAgentes) setLoading(true);
      const json = await apiFetch(
        `/agentes_workspace?workspace_id=${workspaceId}&per_page=100`
      );
      const rows = (json.data || []).map(normalizeWorkspaceAgent);
      setData(rows);
      writeModuleCache(AGENTES_CACHE_KEY, rows);
    } catch (error) {
  ...
  const fetchAseguradoras = useCallback(async () => {
    try {
      if (aseguradoras.length) return;
      const json = await apiFetch('/aseguradoras?per_page=100');
      const rows = json.data || [];
      setAseguradoras(rows);
      writeModuleCache(ASEG_CACHE_KEY, rows);
    } catch (error) {
  ...
  useEffect(() => {
    if (!isAddModalOpen && !agentToEdit) return;
    fetchAseguradoras();
  }, [isAddModalOpen, agentToEdit, fetchAseguradoras]);
